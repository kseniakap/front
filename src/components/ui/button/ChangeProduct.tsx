...
const ChangeProduct:FC<{product: IProduct}> = ({product})=> {
  const {register: formRegister, handleSubmit, formState:{errors}, reset, control} = useForm<TypeProductData>({
    mode: "onChange"
  })
  ...
  if(isSuccess) return (
    <div className={st.publish}>
      <p>Товар изменен</p>
      <Link href= "http://localhost:3000/admin/products">Вернуться на страницу админа <IoIosArrowForward/></Link>
    </div>
  )  
